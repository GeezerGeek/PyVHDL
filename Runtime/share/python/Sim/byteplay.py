...
#------------------------------------------------------------------------------
#
# Revised by VEM - 1/10/14 
#
#  1.  Modify Label class with a zcode address attribute 
#
#  2.  Modify LOAD_CONST, to accommodate StdLogic & SLV constants by changing
#      the comparison function passed to index() to the function stored in the
#      extended_eq module level variable
#      
#------------------------------------------------------------------------------
...
# PyDev python parser doesn't recognize these globals
...
# Allows loading a function to replace operator.is_ for LOAD_CONST 
extended_eq = operator.is_

# Translate text format bytecode list to encoded format
def encode(text_bc):
    result = []
    for inst, arg in text_bc:
        if isinstance(inst, Label):
            result.append((inst, arg))
        else:
            result.append((opmap[inst], arg))
...
class Label(object):
    ...
    def __init__(self, zad = -1, tag = 1):       
        # The zcode address of this label as an int
        self.zad = zad
        self.tag = tag # 1 or 2
    ...
    def __repr__(self):
        return 'Label psudo-op%d at %d' % (self.tag, self.zad)
...
#                label_pos[op] = pos
        ...
        # What we compute - for each opcode, is it's stack state, as an n-tuple.
            ...
            if isinstance(op, Label):
                ...
                if stacks[pos] is None:
                    ...
                    if stacks[pos] != curstack:
                        raise ValueError, "Inconsistent code at %d: sp %s, sc %s" % (pos, stacks[pos], curstack)
                    return
    ...
    def to_code(self):
        """Assemble a Python code object from a byteplay Code object."""
                ...
                if op in hasconst:
                    if isinstance(arg, Code) and i < len(self.code)-1 and \
                       self.code[i+1][0] in hascode:
                        arg = arg.to_code()
                    arg = index(co_consts, arg, extended_eq)
        ...
        # Here is where the code object is created
#        print 'co_lnotab', co_lnotab
#        print 'co_consts', co_consts
#        print 'co_names ', co_names
#        print 'co_varnames', co_varnames
#        print 'co_nlocals', co_nlocals
#        print'co_cellvars', co_cellvars
